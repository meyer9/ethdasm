...
class Parser:
    ...
    @staticmethod
    def __parse_ops(contract_code) -> [Instruction]:
        """
    ...
    def __optimize_math(instructions: [Instruction]) -> [Instruction]:
        i = 0
        while i < len(instructions):
            equiv_func = instructions[i].instruction.get('equivalent_function')
            if instructions[i].arguments and equiv_func and len(instructions[i].arguments) == instructions[i].instruction['removed']:
                ...
                instructions[i] = Instruction(oc.OPCODES[op_code], instructions[i].address, [equiv])
            i += 1
        return instructions
